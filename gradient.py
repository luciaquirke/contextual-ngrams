...
def get_checkpoints_df(
    model_name: str, num_checkpoints: int, lang_data: dict, hook_name="post"
):
    ...
    with tqdm(total=len(checkpoints) * model.cfg.n_layers) as pbar:
        for checkpoint in checkpoints:
            ...
            for layer in range(model.cfg.n_layers):
                ...
                bwd_activations_german = get_backward_activations(
                    lang_data["de"][:40], model, layer, neurons
                )
                bwd_activations_english = get_backward_activations(
                    lang_data["en"][:60], model, layer, neurons
                )
                check_point_df = pd.DataFrame(
                    {
                        "GermanGradients": bwd_activations_german,
                        "EnglishGradients": bwd_activations_english,
                        "Neuron": neurons,
                        "Layer": layer,
                        "Checkpoint": checkpoint,
                        "Label": [f"L{layer}N{i}" for i in neurons],
                    }
                )
    ...
    checkpoint_df["GradientDiff"] = (
        checkpoint_df["EnglishGradients"] - checkpoint_df["GermanGradients"]
    )
...
def save_figures(model_name: str, checkpoint_df: pd.DataFrame, output_path: Path):
    ...
    fig = px.scatter(
        checkpoint_df.groupby("Label").mean().reset_index(),
        x="GermanGradients",
        y="NonGermanGradients",
        color="Checkpoint",
        height=800,
        width=1000,
        hover_name="Label",
        title="German vs NonGerman Backward Activations",
    )
    axis_range = [-0.00007, 0.00007]
    fig.update_xaxes(scaleanchor="y", scaleratio=1)

    fig.save_image(
        output_path.joinpath("german_vs_non_german_backward_activations.png")
    )

    x = (
        checkpoint_df.groupby("Label")
        .mean()["GradientDiff"]
        .sort_values(ascending=False)
    )
    fig = px.histogram(
        x,
        title="Mean (NonGerman - German) Gradient per Neuron",
    ...
    fig = px.line(
        checkpoint_df[checkpoint_df["Label"] == "L0N341"],
        y=["GradientDiff", "NonGermanGradients", "GermanGradients"],
        x="Checkpoint",
        width=800,
        title="Gradient L3N669 between NonGerman and German",
        facet_col="Layer",
        facet_col_wrap=3,
    )
    ...
    fig = px.line(
        checkpoint_df[checkpoint_df["Label"].isin(good_neurons)],
        y="GradientDiff",
        x="Checkpoint",
        color="Label",
        width=800,
        title="Gradient difference NonGerman-German (post)",
    )
    fig.save_image(output_path.joinpath("top_ctx_neurons_gradient_diff_post.png"))

    baseline_neurons = [
        f"L{random.randint(0, model.cfg.n_layers)}N{random.randint(0, model.cfg.d_mlp)}"
        for _ in range(25)
    ]

    fig = px.line(
        checkpoint_df[checkpoint_df["Label"].isin(baseline_neurons)],
        y="GermanGradients",
        x="Checkpoint",
        color="Label",
        width=800,
        title="German gradients random neurons",
    )
    fig.save_image(output_path.joinpath("random_ctx_neurons_gradient_diff_post.png"))

    checkpoint_df[["Checkpoint", "GermanGradients", "NonGermanGradients"]].groupby(
        ["Checkpoint"]
    ).mean().reset_index().head()

    grouped_df = (
        checkpoint_df[["Checkpoint", "GermanGradients", "NonGermanGradients", "Layer"]]
        .groupby(["Checkpoint", "Layer"])
        .mean()
        .reset_index()
    )
    grouped_df = grouped_df.melt(
        id_vars=["Checkpoint", "Layer"],
        value_vars=["GermanGradients", "NonGermanGradients"],
        var_name="Language",
        value_name="Gradient",
    )
    grouped_df["LayerLanguage"] = grouped_df.apply(
        lambda row: f"L{row['Layer']} {row['Language']}", axis=1
    )
    ...
    grouped_df = (
        checkpoint_df.loc[
            checkpoint_df["Layer"] > 2,
            ["Checkpoint", "GermanGradients", "NonGermanGradients", "Layer"],
        ]
        .groupby(["Checkpoint"])
        .mean()
        .reset_index()
    ...
    fig = px.line(
        grouped_df,
        x="Checkpoint",
        y=["GermanGradients", "NonGermanGradients"],
        title="Mean gradients for Layers 3, 4, and 5",
    )
