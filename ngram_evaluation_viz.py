...
def process_data(model_name: str, output_dir: Path, image_dir: Path) -> None:
    ...
    fig.update_layout(title_text="Contextual trigram losses with and without ablating the German neuron")
    fig.update_layout(
        #yaxis=dict(type='log'),
        #yaxis2=dict(type='linear')
    ...
    fig.update_layout(title_text="Indirect and direct trigram ablation losses from ablating the German neuron")
    fig.update_layout(
        #yaxis=dict(type='log'),
        #yaxis2=dict(type='linear')
        yaxis=dict(range=[0, 13]),
