...
def process_data(model_name: str, output_dir: Path, image_dir: Path) -> None:
    ...
    context_effect_df["Ablation increase"] = (
        context_effect_df["Ablated Loss"] - context_effect_df["Original Loss"]
    )
    context_effect_df["Ablation increase (fraction)"] = (
        context_effect_df["Ablation increase"] / context_effect_df["Original Loss"]
    )
    ...
    fig.update_layout(title=f"German trigram (N={num_trigrams}) losses over checkpoints", xaxis_title="Checkpoint", yaxis_title="Loss", font=dict(size=24, family="Times New Roman, Times, serif"),)
    ...
    fig.update_layout(
        title_text="Indirect and direct trigram ablation losses from ablating the German neuron",
        xaxis_title="Checkpoint",
        yaxis_title="Loss",
        yaxis=dict(range=[0, 13]),
        font=dict(size=24, family="Times New Roman, Times, serif"),
    ...
    fig.write_image(
        image_dir.joinpath("trigram_losses_direct_indirect.png"), width=2000
    )
