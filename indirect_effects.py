...
def process_data(model_name: str, output_dir: Path, data_dir: Path):
    ...
    for checkpoint in tqdm(range(0, len(checkpoints), 5)):
        ...
        data.append(
            [
                checkpoint,
                np.mean(original_metrics),
                np.mean(ablated_metrics) - np.mean(original_metrics),
                np.mean(direct_effect_metrics) - np.mean(original_metrics),
                np.mean(indirect_effect_metrics) - np.mean(original_metrics),
            ]
        )
